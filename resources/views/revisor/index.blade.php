		...
		@if ($article_to_check)
			<div class="row pt-5 gap-3">
				...
				<div class="col-12 col-md-6 col-lg-6 d-flex flex-column justify-content-between">
					<div>
						<div class="form-label accent mt-3">Titolo</div>
						<h2 class="mb-3">
						...
						<div class="row gap-3 justify-content-between">
							<div class="col-auto d-flex gap-5">
								<div>
									<div class="form-label accent">Autore</div>
									<div>{{ $article_to_check->user->name }}</div>
								</div>
								<div>
									<div class="form-label accent">Prezzo</div>
									<div>{{ $article_to_check->price }}€</div>
								</div>
								<div>
									<div class="form-label accent">Categoria</div>
							...
							<div class="col-auto">
								<div class="d-flex gap-4">
									<form action="{{ route('reject', ['article' => $article_to_check]) }}" method="POST">
										@csrf
										@method('PATCH')
										<button class="btn btn-danger py-2 px-5">Rifiuta</button>
									</form>
									<form action="{{ route('accept', ['article' => $article_to_check]) }}" method="POST">
										@csrf
										@method('PATCH')
										<button class="btn btn-success py-2 px-5">Accetta</button>
									</form>
								</div>
							</div>
						</div>					
						<hr>
						<div class="form-label accent mt-4">Messaggio</div>
						<p class="mt-2">
		...
		@else
			<div class="row">
				<div class="col-auto">
					...
					<a href="{{ route('homepage') }}" class="mt-5 btn btn-primary-accent">Torna all'homepage</a>
				</div>
			</div>
		@endif
	</div>
